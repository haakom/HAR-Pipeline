...
def run_experiments(experiment_collection_path, experiments=None, seed=None):
    ...
    top_statistics_folder = os.path.join(VAGESHAR_ROOT, "loso_statistics", all_experiments_id)
    top_images_folder = os.path.join(VAGESHAR_ROOT, "loso_images", all_experiments_id)
    ...
    print("Seed: ", seed)
    if not os.path.exists(top_statistics_folder):
    ...
    for experiment_name in experiments:
        ...
        # Todo: Remove the hard booleans here
        ...
        # Todo: Remove hard window length here
        window_length = 3  # experiment_config.getfloat("window_length")
        train_overlap = experiment_config.getfloat("train_overlap")
        test_overlap = experiment_config.getfloat("test_overlap")

        train_data_loader = Dl(sample_rate=sample_frequency, window_length=window_length,
                               degree_of_overlap=train_overlap)
        test_data_loader = Dl(sample_rate=sample_frequency, window_length=window_length,
        ...
        if run_ordinary_rfc_test:
            ...
            for subject_id in test_subject_ids:
                ...
                print(subject_id)
                subject_x_test, subject_y_test = test_sensor_dict[subject_id], test_label_dict[subject_id]
                ...
                x_train_ = other_x_train
                y_train_ = other_y_train
...
if __name__ == "__main__":
    config_paths = [[os.path.join(VAGESHAR_ROOT, "configs", "train_stroke_test_healthy.cfg"), None]]
